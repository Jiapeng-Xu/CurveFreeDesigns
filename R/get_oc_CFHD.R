#'
#' Generate operating characteristics for Curve-Free Bayesian Design (CFHD)
#'
...
#'
#' @details The operating characteristics of the CFHD design are generated by simulating trials under the prespecified true toxicity and efficacy probabilities.
...
#'
#' @return \code{get_oc_CFHD} returns the operating characteristics of the CFHD design as a list,
#'         including:
...
get_oc_CFHD <- function (pTox, pEff, var.ratio, var.ratio.E, target, T.max, E.min, n.min.mtd, n.max.mtd, n.min.int, n.max.int, confirmation_cohort, n.cohort, n.sim, seed,
  ...
  if (n.cohort <= 0) {
    stop("Cohort size need to be greater than 0")
  }
  ...
  # initialize storage
  #--------------------
  sample.size <- numeric(n.sim)      # sample size for each simulation; update after enrolling new patient
  is.BED      <- numeric(n.sim)      # finding a BED interval or not per simulation; update after final decision of BEDs is made
