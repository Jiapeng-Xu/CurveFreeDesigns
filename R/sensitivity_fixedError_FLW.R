#'
#' Conduct sensitivity analysis of Curve-Free Design for Maximum Tolerated Dose in One-Agent Trials based on fixed errors
#'
...
#'
#' @return \code{sensitivity_randomError_FLW()} returns the operating characteristics of the proposed design as a list,
#'         including:
...
sensitivity_fixedError_FLW <- function (pTox, mis_pTox, var.ratio, target, T.max, n.min.mtd, n.max.mtd, n.sim, seed, alpha = 1, eta = 1, p1 = 0.1, p2 = 0.1) {
  ...
  for (s in 1:n.sim) {
    ...
    #
    #if (sample.size[s] >= n.max.mtd && is.stop == 0) {}
